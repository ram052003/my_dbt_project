{{
  config(
    ...
    unique_key=['I_BUS','',' D_EFFCTV_STRT'],
    merge_update_columns=['I_BUS','T_STMP_UPD','D_EFFCTV_STRT','D_EFFCTV_END']
  )
}}
...
Xfr_dlrbank_RmvDup_inOut as (
	
Select 
I_BUS as I_BUS,
Current_timestamp ( ) as T_STMP_UPD,
D_EFFCTV_STRT_5 as D_EFFCTV_STRT,
D_SRCE_UPD as D_EFFCTV_STRT_MAX 
From DSstgVar_XFR_DLRBANKOut1 as DSstgVar_XFR_DLRBANKOut1 Where D_EFFCTV_STRT_5 IS NULL = 0 AND StgDel = 0),

RMV_DUP_DLRBANKOut as (
	Select DISTINCT
	LAST_VALUE ( D_EFFCTV_STRT ) OVER ( PARTITION BY I_BUS ORDER BY I_BUS ) as D_EFFCTV_STRT,
	 LAST_VALUE ( D_EFFCTV_END ) OVER ( PARTITION BY I_BUS ORDER BY I_BUS ) as D_EFFCTV_END,
	 I_BUS,
	 LAST_VALUE ( T_STMP_UPD ) OVER ( PARTITION BY I_BUS ORDER BY I_BUS ) as T_STMP_UPD 
From Xfr_dlrbank_RmvDup_inOut as Xfr_dlrbank_RmvDup_inOut)

Select 
	I_BUS as I_BUS,
	T_STMP_UPD as T_STMP_UPD,
	D_EFFCTV_STRT as D_EFFCTV_STRT,
	D_EFFCTV_END as D_EFFCTV_END 
From RMV_DUP_DLRBANKOut as RMV_DUP_DLRBANKOut